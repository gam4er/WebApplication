...
5. Кнопка **Deactivate** отключает VPP без перезапуска приложения.
...
| Возможность | Где используется |
|---|---|
| **`HostingEnvironment.RegisterVirtualPathProvider`** | Регистрация кастомного провайдера в рантайме по нажатию кнопки — без перезапуска AppDomain. |
